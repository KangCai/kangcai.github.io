...
* 解法
    ...
    * O(nloglogn): 设 n 为 10，则候选布尔数组为 <F,F,T,T,T,T,T,T,T,T>，其中 T 表示该位置是质数。从 2 开始，更新候选布尔数组 <F,F,T,T,F,T,F,T,F,T,F>；再依据 3 更新候选布尔数组 <F,F,T,T,F,T,F,T,F,F,F>，再从新布尔数组中后续的为 T 的 5 开始，... 总时间复杂度是 O(n/2+n/3+n/5+n/7+n/11)，渐进上限是 O(nloglogn)
