...
* 算法：在策略部分，我们发现目标是求目标函数的最优解（最小值），**算法部分就是求最优解的解决方法**。高中知识告诉我们，通常来说，最小值是极值点，而连续函数的极值点可以通过求一阶导数并令导数等于0。机器学习中常用的算法有最小二乘法、迭代法，最小二乘法针对线性模型，而迭代法针对任意模型。
...
这个函数就称为**损失函数(loss function)**。损失函数越小，就代表模型对某个样本拟合得越好。期望风险函数是损失函数的期望，这是由于我们输入输出遵循一个联合分布，但是这个联合分布是未知的，所以无法计算。但是我们是有历史数据的，就是我们的训练集，关于训练集的平均损失称作**经验风险(Empirical risk)**，我们常用经验风险替代期望风险，因为根据[大数定理](https://zh.wikipedia.org/wiki/%E5%A4%A7%E6%95%B0%E5%AE%9A%E5%BE%8B)，当样本容量趋于无穷时，经验风险也就趋于期望风险，经验风险可表示为，
...
还记得高中课本里关于线性回归（线性拟合）某一节不要求掌握的公式嘛？对于坐标为(x,y)的n个样本，假设拟合直线为y=ax+b, 通过使<img src="http://latex.codecogs.com/gif.latex?\sum_{i=1}^{n}(y_i-ax_i-b)^2" title="\sum_{i=1}^{n}(y_i-ax_i-b)^2" />最小，然后对a和b分别求偏导，得到的了a和b关于n个样本的表示，这种方法就叫做最小二乘法，这是一种基于高斯分布假设的回归方法，具体以后的文章再细说。

##### 4.2 迭代法

迭代法家族还有多个著名成员：牛顿法（Newton iteration method）、梯度下降（Gradient descent method）、高斯-牛顿法（Gauss-Newton iteration method）、Levenberg-Marquardt方法（动态选择使用梯度下降法还是高斯牛顿法）。

* 牛顿法：牛顿法首先则是先设定初始值<img src="http://latex.codecogs.com/gif.latex?x_0" title="x_0" />，将问题转化为求 <img src="http://latex.codecogs.com/gif.latex?f'(x_0)=0" title="f'(x_0)=0" /> 这个方程的根，然后求得的根作为下一次迭代的 _x_ 。下图是牛顿法求极值的经典图，

<img src="https://kangcai.github.io/img/in-post/post-ml/Newton iteration method.png"/>


* 梯度下降法：梯度是上升最快的方向，那么如果逆着上升最快的方向就是此刻下降最快的方向，所以梯度下降法通常也称最速下降法（Steepest descent method）

<img src="http://latex.codecogs.com/gif.latex?\theta^{t+1}\leftarrow \theta^t - \eta  \triangledown L(\theta) "/>

其中<img src="http://latex.codecogs.com/gif.latex?\eta"/>叫学习率（Learning rate，后面会经常接触），梯度下降法在机器学习中使用的极其广泛，有多个种类，我们在《机器学习·总览篇VII 三要素之算法》一文中将对梯度下降法一族进行详细的介绍。
