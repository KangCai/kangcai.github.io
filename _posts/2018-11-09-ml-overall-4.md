...
（2）当牛顿法用于机器学习时，目标是求解 <img src="http://latex.codecogs.com/gif.latex?L'(\theta)=0"/> 由于公式为 <img src="http://latex.codecogs.com/gif.latex?\theta^{t+1}\leftarrow \theta^t - \frac{L'(\theta)}{L''(\theta)} "/> 。牛顿法的缺点是存在求二阶导高消耗计算、二阶导数根本不存在无法计算、初始值离局部极小值太远则很可能无法收敛等问题，所以在机器学习中一般不会直接使用牛顿法。
...
GN是对牛顿法的改进，GN将目标函数进行泰勒展开，



这样有两个好处，第一是线性、第二是只需要一阶微分。引入了海森矩阵（Hessian matrix）。缺点是GN之反矩阵不存在时，而且在接近极值点时，Hessian矩阵变得非常小，可以类比成缩小了梯度下降法的学习率，。
...
**梯度下降法**在机器学习中使用得最为广泛，而且其子类及其扩展类别多种多样，我们在《机器学习·总览篇VII 三要素之算法》一文中将对梯度下降法一族进行详细的介绍。
